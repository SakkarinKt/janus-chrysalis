...
```ts
export function rewardLoss(predicted: tf.Tensor2D, target: tf.Tensor2D): tf.Scalar {
  return tf.tidy(() => tf.mean(tf.square(tf.sub(predicted, target))) as tf.Scalar);
}
```

Mean squared error between the predicted and actual scalar reward — the same simplification
`reconstructionLoss` already uses (negative log-likelihood under a unit-variance isotropic
Gaussian, up to an additive constant and a `0.5` factor), specialized to a `[batch, 1]` target
instead of `[batch, observationSize]`. Not implemented by calling `reconstructionLoss` with
`observationSize: 1` — kept as a distinct named function per this codebase's existing
one-loss-per-head convention (`reconstructionLoss`, `continueLoss`, now `rewardLoss`), so a future
...
- **Loss total**: `reconstructionLoss + klBalancedLoss(...).total + continueLoss(...) +
  rewardLoss(predicted, target)`, unweighted (coefficient 1) — `0011`'s own precedent for adding a
  DreamerV3 loss term without inventing a new coefficient, applied a second time.
...
Two corrections `0013` needs to carry once this head exists — noted here rather than silently left
for whoever implements `Actor`/`Critic` to rediscover:

1. **`imaginationTrainStep`'s `rewards.push(rewardHead.predict(...))` line** must convert that
   tensor to a plain number before use (`.dataSync()[0]`, the same conversion `0013`'s own
   `advantage` computation already applies to `values[t]`) — `computeLambdaReturns`'s `rewards`
   parameter is `number[]` per `0012`'s pinned contract, same as `values`/`continues`.
...
- `test/model/losses.test.ts` gains: `rewardLoss` is near-zero when `predicted === target` and
  large for a confidently-wrong prediction; gradient flows into `predicted` (`tf.variableGrads`
  against a `tf.Variable` prediction is nonzero).
