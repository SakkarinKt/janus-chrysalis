...
/** Arbitrary fixed reward scale — shape/gradient-wiring tests, not a claim about a real environment's range. */
const REWARD_SCALE = 4.0;
/** Arbitrary fixed reward, within REWARD_SCALE's range — same "shape check, not a real signal" caveat as OBSERVATION. */
const REWARD = -0.5;
...
test("WorldModel: seed reproduces identical initial weights (cell + decoder + continueHead + rewardHead) across independent instances; different seeds diverge — the fix for PR #45's review (gate (b) isn't readable until init is seeded or paired)", () => {
  const weightsOf = (wm: WorldModel) => [
    ...
    ...wm.rewardHead.trainableWeights().map((w) => Array.from(w.dataSync())),
  ];

  const a = new WorldModel({ rssm: CONFIG, observationSize: OBSERVATION_SIZE, rewardScale: REWARD_SCALE, seed: 1001 });
  const b = new WorldModel({ rssm: CONFIG, observationSize: OBSERVATION_SIZE, rewardScale: REWARD_SCALE, seed: 1001 });
  assert.deepEqual(weightsOf(a), weightsOf(b), "same seed must draw identical initial weights");
  a.dispose();
  b.dispose();

  const c = new WorldModel({ rssm: CONFIG, observationSize: OBSERVATION_SIZE, rewardScale: REWARD_SCALE, seed: 1001 });
  const d = new WorldModel({ rssm: CONFIG, observationSize: OBSERVATION_SIZE, rewardScale: REWARD_SCALE, seed: 1002 });
...
test("WorldModel: state (deterministic + stochastic) changes every step, regardless of train", () => {
  ...
  wm.step(Action.Up, OBSERVATION, rng, true, false, REWARD);
  ...
  wm.step(Action.Down, OBSERVATION, rng, false, false, REWARD);
...
test("WorldModel: train=true changes at least one trainable weight; train=false leaves every weight bit-identical", () => {
  const wm = new WorldModel({ rssm: CONFIG, observationSize: OBSERVATION_SIZE, rewardScale: REWARD_SCALE, lossConfig: { freeBits: 0 } });
  const rng = new Rng(1);
  const weightsOf = () => wm.cell.trainableWeights().map((w) => Array.from(w.dataSync()));

  const beforeFrozen = weightsOf();
  wm.step(Action.Up, OBSERVATION, rng, false, false, REWARD);
...
test("WorldModel: reconstructionLoss + klLoss + continueLoss + rewardLoss equals loss, every step", () => {
  const wm = new WorldModel({ rssm: CONFIG, observationSize: OBSERVATION_SIZE, rewardScale: REWARD_SCALE });
  const rng = new Rng(1);

  for (const train of [true, false, true]) {
    const result = wm.step(Action.Up, OBSERVATION, rng, train, false, REWARD);
    assert.ok(
      Math.abs(
        result.reconstructionLoss + result.klLoss + result.continueLoss + result.rewardLoss - result.loss,
      ) < 1e-4,
      `expected reconstructionLoss + klLoss + continueLoss + rewardLoss ~= loss, got ${result.reconstructionLoss} + ` +
        `${result.klLoss} + ${result.continueLoss} + ${result.rewardLoss} != ${result.loss}`,
    );
  }
...
test("WorldModel: train=true changes at least one rewardHead weight; train=false leaves every rewardHead weight bit-identical (docs/explainers/0014)", () => {
  const wm = new WorldModel({ rssm: CONFIG, observationSize: OBSERVATION_SIZE, rewardScale: REWARD_SCALE, lossConfig: { freeBits: 0 } });
  const rng = new Rng(1);
  const rewardHeadWeightsOf = () => wm.rewardHead.trainableWeights().map((w) => Array.from(w.dataSync()));

  const beforeFrozen = rewardHeadWeightsOf();
  wm.step(Action.Up, OBSERVATION, rng, false, false, REWARD);
  const afterFrozen = rewardHeadWeightsOf();
  assert.deepEqual(beforeFrozen, afterFrozen, "train=false must not move any rewardHead weight");

  const beforeTrain = rewardHeadWeightsOf();
  wm.step(Action.Up, OBSERVATION, rng, true, false, REWARD);
  const afterTrain = rewardHeadWeightsOf();
  assert.notDeepEqual(beforeTrain, afterTrain, "train=true must move at least one rewardHead weight");
...
test("WorldModel: repeated identical-input training steps drive reconstructionLoss down specifically (freeBits: 0 isolates it from the KL floor, same convention as the combined-loss test below)", () => {
  ...
  for (let i = 0; i < 150; i++) {
    reconLosses.push(wm.step(Action.Up, OBSERVATION, rng, true, false, REWARD).reconstructionLoss);
  }
...
test("WorldModel: repeated identical-input, identical-done training steps drive continueLoss down specifically, in both done directions (docs/explainers/0011 — the target is a constant 0 or 1 per direction, so this is a shape/gradient-wiring check, not a claim about learning a real termination signal)", () => {
  for (const done of [false, true]) {
    ...
    for (let i = 0; i < 150; i++) {
      continueLosses.push(wm.step(Action.Up, OBSERVATION, rng, true, done, REWARD).continueLoss);
    }
...
test(
  ...
  () => {
    for (const [done, expectedTarget] of [[false, 1], [true, 0]] as const) {
      ...
      const result = wm.step(Action.Up, OBSERVATION, rng, false, done, REWARD);
...
test("WorldModel: reset() returns to a fresh zero-filled state and disposes the previous one", () => {
  ...
  const rng = new Rng(1);
  wm.step(Action.Up, OBSERVATION, rng, true, false, REWARD);
  wm.step(Action.Down, OBSERVATION, rng, true, false, REWARD);
...
test("WorldModel: repeated identical-input training steps drive the combined (reconstruction + KL-balanced + continue) loss down (freeBits: 0, isolating signal from the KL floor per test/model/losses.test.ts's convention — coarse 'does it learn' check, not a convergence guarantee)", () => {
  ...
  const losses: number[] = [];
  for (let i = 0; i < 150; i++) {
    losses.push(wm.step(Action.Up, OBSERVATION, rng, true, false, REWARD).loss);
  }
...
test("WorldModel: a warm (post-first-step) run of mixed train/eval steps leaves 0 net tensors beyond the two persisted state tensors (tensor-leak check)", () => {
  ...
  for (let i = 0; i < 20; i++) {
    wm.step(i % 2 === 0 ? Action.Up : Action.Down, OBSERVATION, rng, i % 3 !== 0, false, REWARD);
  }
...
test("WorldModel: a throw from forward() (wrong-length observation, shape mismatch inside cell.posterior) leaves this.state exactly as it was — still usable, not disposed (PR #40 review follow-up 1)", () => {
  ...
  assert.throws(() => wm.step(Action.Up, wrongLengthObservation, rng, true, false, REWARD));
...
test("WorldModel: a throw from forward() with train=false leaks no tensors (tensor-leak check across a caught throw, PR #40 review follow-up 2 — the train=true path also leaks nothing of ours, but tf.variableGrads' own internal tidy drops ~66 intermediates on its own error path regardless of branch, a pre-existing library-level cost outside step()'s control)", () => {
  ...
  const before = tf.memory().numTensors;
  const wrongLengthObservation = OBSERVATION.slice(0, 3);
  assert.throws(() => wm.step(Action.Up, wrongLengthObservation, rng, false, false, REWARD));
  const after = tf.memory().numTensors;
...
test("WorldModel: a throw from decoder.decode() — after the tf.keep() calls above, unlike every other throw case above — leaves this.state unchanged and leaks no tensors (PR #41 review non-blocking note 1: nextDeterministic?.dispose()/nextStochastic?.dispose() at the catch block was, until this test, exercised by zero tests)", () => {
  ...
  assert.throws(() => wm.step(Action.Down, OBSERVATION, rng, false, false, REWARD), /stubbed decode failure/);
...
test("WorldModel: a NaN-valued observation with train=false throws WorldModelNaNError, leaves recurrent state and weights untouched and leaks no tensors, and a subsequent clean step recovers fully — the NaN-halt invariant (loop/GOAL.md priority 5, docs/explainers/0009)", () => {
  ...
  assert.throws(
    () => wm.step(Action.Up, nanObservation, rng, false, false, REWARD),
    (err: unknown) =>
  ...
  const result = wm.step(Action.Down, OBSERVATION, rng, true, false, REWARD);
...
test("WorldModel: a NaN-valued observation with train=true throws WorldModelNaNError and leaves recurrent state unchanged and leak-free, but — documented limitation, docs/explainers/0009 — cannot undo the optimizer step forward() already applied before the throw, so weights are left NaN-corrupted", () => {
  ...
  assert.throws(
    () => wm.step(Action.Up, nanObservation, rng, true, false, REWARD),
    (err: unknown) =>
      err instanceof WorldModelNaNError && Number.isNaN(err.loss) && Number.isNaN(err.reconstructionLoss),
  );
